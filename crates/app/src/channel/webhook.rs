...
async fn process_webhook_request(state: &WebhookServeState, body: Bytes) -> CliResult<Response> {
    ...
    state
        ...
        .mark_run_start()
        .await
        .map_err(|error| format!("webhook runtime start failed: {error}"))?;
    ...
    let process_result = async {
        ...
        build_webhook_inline_response(state, reply.as_str())
    }
    .await;
