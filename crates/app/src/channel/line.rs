...
async fn process_line_webhook(state: &LineServeState, body: Bytes) -> CliResult<()> {
    let payload: serde_json::Value = serde_json::from_slice(body.as_ref())
        .map_err(|error| format!("parse line webhook payload failed: {error}"))?;
    let events = payload
        .get("events")
    ...
    for event in &events {
        process_line_event(state, event).await?;
    }

    Ok(())
}

async fn process_line_event(state: &LineServeState, event: &serde_json::Value) -> CliResult<()> {
    ...
    let message = event
        ...
        .ok_or_else(|| "line webhook message event missing message object".to_owned())?;
    ...
    let reply_token = event
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| "line webhook event missing replyToken".to_owned())?;
    let inbound_text = message
        .get("text")
        .and_then(serde_json::Value::as_str)
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| "line webhook text message missing text".to_owned())?;
    let source = event
        .get("source")
        .and_then(serde_json::Value::as_object)
        .ok_or_else(|| "line webhook event missing source object".to_owned())?;
    let conversation_id = resolve_line_conversation_id(source)
        .ok_or_else(|| "line webhook source missing address".to_owned())?;
    ...
    state
        ...
        .mark_run_start()
        .await
        .map_err(|error| format!("line runtime start failed: {error}"))?;
    ...
    let process_result = async {
        ...
        send_line_reply(state, reply_token, reply.as_str()).await
    }
    .await;
