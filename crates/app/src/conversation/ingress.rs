...
mod tests {
    ...
    fn feishu_callback_private_ingress_is_injected_only_for_feishu_tools() {
        ...
        assert_eq!(
            injected.payload["_loong"]["feishu_callback"]["callback_token"],
            "callback-secret-1"
        );
        assert_eq!(
            injected.payload["_loong"]["feishu_callback"]["operator_open_id"],
            "ou_operator"
        );
        assert_eq!(
            injected.payload["_loong"]["feishu_callback"]["deferred_context_id"],
            "evt_callback_1"
        );
        assert_eq!(
        ...
        assert!(untouched.payload.get("_loong").is_none());
    }
