...
fn render_skills_cli_text_surfaces_browser_preview_guidance() {
    ...
    assert!(rendered.contains("- summarize a page: loong ask --config '/tmp/loong'\"'\"'s config.toml' --message 'Use the browser companion preview to open https://example.com, snapshot the page, and summarize what is visible.'"));
}
