...
mod tests {
    ...
    #[test]
    fn non_interactive_commands_keep_default_logging_behavior() {
        let command = Commands::Welcome;
        assert_eq!(quiet_interactive_log_directive(&command, false), None);
    }

    #[test]
    fn interactive_chat_suppresses_env_compat_warning_noise() {
        let command = Commands::Chat {
            config: None,
            session: None,
            acp: false,
        ...
        assert!(!suppress_env_compat_warning(&Commands::Welcome));
    }
