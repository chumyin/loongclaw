...
pub(crate) fn onboarding_model_selection_context(
    ...
    let allows_auto_fallback_hint =
        provider.explicit_model().is_none() && !preferred_fallback_models.is_empty();
...
pub(crate) fn resolve_onboarding_model_prompt_default(
    ...
    let Some(recommended_model) = provider.kind.recommended_onboarding_model() else {
        return Ok(configured_model);
    };
...
fn recommended_onboarding_model_for_context(
    ...
    Some(recommended_model.to_owned())
}
