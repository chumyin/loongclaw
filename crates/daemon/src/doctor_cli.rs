    ...
    if options.skip_model_probe {
        ...
        match mvp::provider::fetch_available_models(&config).await {
            ...
            Err(error) => {
                ...
                let collect_route_probe = matches!(
                    probe_failure.recovery_kind,
                    provider_model_probe_policy::ProviderModelProbeRecoveryKind::TransportFailure
                );
                let check = doctor_check_from_provider_model_probe_failure(probe_failure);
                checks.push(check);
                if collect_route_probe
...
fn build_doctor_next_steps_with_path_env(
    ...
    if has_provider_model_probe_failure {
        ...
        if provider_model_probe_transport_failure {
            ...
            let provider_model_probe_recovery_kind =
                provider_model_probe_policy::provider_model_probe_configured_recovery_kind(config);

            match provider_model_probe_recovery_kind {
                provider_model_probe_policy::ProviderModelProbeConfiguredRecoveryKind::RequiresExplicitModel {
                ...
                provider_model_probe_policy::ProviderModelProbeConfiguredRecoveryKind::ExplicitModel {
                    ..
                }
                | provider_model_probe_policy::ProviderModelProbeConfiguredRecoveryKind::PreferredModels {
                    ..
                } => {
                    push_unique_step(
                        &mut steps,
