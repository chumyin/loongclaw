...
pub enum Commands {
    ...
    #[command(
        about = "List channel surfaces or resolve one channel/route-session into operator-facing target details",
        long_about = "List compiled channel surfaces, aliases, access-policy summaries, and readiness status.\n\nUse `loong channels` to inspect the configured inventory, or add `--resolve <channel-or-route-session>` to explain one catalog alias/id or one known runtime-backed route session before sending or re-routing traffic. `--json` keeps the same command on a machine-readable surface for both list and resolve flows."
    )]
    Channels {
        #[arg(long)]
        config: Option<String>,
    ...
    /// Revoke one approved channel-side participant pairing
    ...
    /// Clear pending channel-side pairing requests within one scoped account or participant
    ClearChannelPairings {
        #[arg(long)]
        config: Option<String>,
        #[arg(long)]
        channel_id: String,
