...
TRAP_MODES="${4:-${LOONG_STRESS_WASM_TRAPS_MODES:-auto}}"
LOCKED="${LOONG_STRESS_LOCKED:-true}"
CONTINUE_ON_FAILURE="${LOONG_STRESS_CONTINUE_ON_FAILURE:-false}"
...
if [[ "$LOCKED" != "true" && "$LOCKED" != "false" ]]; then
  echo "invalid LOONG_STRESS_LOCKED: $LOCKED (expected true|false)" >&2
  exit 2
fi

if [[ "$CONTINUE_ON_FAILURE" != "true" && "$CONTINUE_ON_FAILURE" != "false" ]]; then
  echo "invalid LOONG_STRESS_CONTINUE_ON_FAILURE: $CONTINUE_ON_FAILURE (expected true|false)" >&2
  exit 2
fi
...
run_mode() {
  ...
  for run_index in $(seq 1 "$ROUNDS"); do
    ...
    if [[ "$trap_mode" == "auto" ]]; then
      if ! env -u LOONG_WASM_SIGNALS_BASED_TRAPS "${cmd[@]}" >"$log_file" 2>&1; then
        status="FAIL"
      fi
    elif ! LOONG_WASM_SIGNALS_BASED_TRAPS="$trap_mode" "${cmd[@]}" >"$log_file" 2>&1; then
      status="FAIL"
    fi
