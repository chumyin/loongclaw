...
if [ -n "${LOONG_RELEASE_DOCS_STRICT:-}" ]; then
    case "${LOONG_RELEASE_DOCS_STRICT}" in
        1|true|TRUE|yes|YES) STRICT_RELEASE_DOCS=1 ;;
        0|false|FALSE|no|NO) STRICT_RELEASE_DOCS=0 ;;
        *)
            echo "FAIL: invalid LOONG_RELEASE_DOCS_STRICT value '${LOONG_RELEASE_DOCS_STRICT}' (expected 0/1)"
            exit 1
            ;;
    esac
