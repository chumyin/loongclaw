...
| Mattermost | Runtime-backed callback | Mattermost REST API + outgoing webhooks | `mattermost.enabled`, `mattermost.server_url`, `mattermost.bot_token` for sends; `mattermost.outgoing_token`, `mattermost.allowed_channel_ids` for serve | `loong mattermost-send`, `loong mattermost-serve` |
